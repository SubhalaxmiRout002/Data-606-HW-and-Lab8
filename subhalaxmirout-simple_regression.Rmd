...
I would use a scatter plot to display the relationship between runs and at_bats. </br>
The relationship looks linear with some outliners.</br>
If I knew a team’s at_bats, I would be comfortable using a linear model to predict the number of runs.
...
There appears to be a positive relationship between runs and at bats. </br>
Correlation coefficient( 0.610627) indicates a positive correlation but not very strong.</br>
The at bats increases number of runs also increases.</br>
There are some outliners present in the data.
...
I ran the function many times and every time I got the same sum of squares: 123721.9.
...
Regression line equation: \[\hat{y} = -85.1566 +  0.3415 * homeruns\]
...
# predict runs with 5578 at-bats
y <- round(-2789.2429 + 0.6305 * 5578)
y
data <- DT::datatable(mlb11)
data
```

  
According to regression line, the manager would predict 728 runs for a team with 5578 at-bats. For actual data, Philadelphia Phillies has a close at-bats of 5579 and runs of 713, which is 15 runs less than the prediction. So this is an overestimation by 15 runs.
...
There is no such pattern in the residuals plot. There is probably a linear relationship between runs and at-bats.
...
The above scatter plot shows linear relationship between hits and runs.
...
Comparing R2 values from 2 models i.e m1 and m2, hits seems to predict runs better than at_bats because m2 has higher R2 value than m1.
...
m3 model (bat_avg vs runs) has best predicting value for the runs because this model has higher $R^2$ value.
...
All 3 models are better predictions for runs. m9 (runs ~ new_obs) has higher $R^2$ than among all. m9 has 93.5% variability accounted for.
...
The relationship looks linear. This indicates a linear relationship between the two variables.
...
The residuals seems mostly normal according to histogram plot.
...
The points fall above and below the line constantly. This seems to be constant variability.
